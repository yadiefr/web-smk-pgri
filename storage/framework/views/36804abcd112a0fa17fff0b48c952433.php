...
<div class="p-3 sm:p-6" x-data="absensiManager()" x-init="init()">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 space-y-4 sm:space-y-0">
        ...
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            <a href="<?php echo e(route('siswa.ketua-kelas.rekap-absensi')); ?>" class="inline-flex items-center justify-center px-4 py-2 bg-cyan-600 text-white text-sm font-medium rounded-lg hover:bg-cyan-700 transition-colors">
    ...
    <!-- Form Absensi -->
    <form method="POST" action="<?php echo e(route('siswa.ketua-kelas.absensi.simpan')); ?>" @submit="onSubmit">
        <?php echo csrf_field(); ?>
...
<?php endif; ?>
                                    ...
                                    <div class="grid grid-cols-2 sm:flex sm:space-x-2 gap-1 sm:gap-0">
                                        <label class="inline-flex items-center">
                                            <input type="radio" 
                                                   ...
                                                   class="sr-only"
                                                   <?php echo e((isset($existingAbsensi[$siswa->id]) && $existingAbsensi[$siswa->id]->status == 'hadir') ? 'checked' : ''); ?>>
                                        ...
                                        <label class="inline-flex items-center">
                                            <input type="radio" 
                                                   ...
                                                   x-model="absensiData[<?php echo e($siswa->id); ?>]"
                                                   class="sr-only"
                                                   <?php echo e((isset($existingAbsensi[$siswa->id]) && $existingAbsensi[$siswa->id]->status == 'sakit') ? 'checked' : ''); ?>>
                                            <span class="px-2 sm:px-3 py-1 text-xs font-medium rounded-full cursor-pointer transition-colors w-full sm:w-auto text-center"
                                        ...
                                        <label class="inline-flex items-center">
                                            <input type="radio" 
                                                   ...
                                                   x-model="absensiData[<?php echo e($siswa->id); ?>]"
                                                   class="sr-only"
                                                   <?php echo e((isset($existingAbsensi[$siswa->id]) && $existingAbsensi[$siswa->id]->status == 'izin') ? 'checked' : ''); ?>>
                                            <span class="px-2 sm:px-3 py-1 text-xs font-medium rounded-full cursor-pointer transition-colors w-full sm:w-auto text-center"
                                        ...
                                        <label class="inline-flex items-center">
                                            <input type="radio" 
                                                   ...
                                                   x-model="absensiData[<?php echo e($siswa->id); ?>]"
                                                   class="sr-only"
                                                   <?php echo e((isset($existingAbsensi[$siswa->id]) && $existingAbsensi[$siswa->id]->status == 'alpha') ? 'checked' : ''); ?>>
                                            <span class="px-2 sm:px-3 py-1 text-xs font-medium rounded-full cursor-pointer transition-colors w-full sm:w-auto text-center"
                                ...
                                <td class="px-3 sm:px-6 py-4 hidden lg:table-cell">
                                    <input type="text" 
                                           ...
                                           value="<?php echo e(isset($existingAbsensi[$siswa->id]) ? $existingAbsensi[$siswa->id]->keterangan : ''); ?>">
                                </td>
                            </tr>
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 pt-4 border-t border-gray-200 space-y-4 sm:space-y-0">
                    <div class="text-gray-600 text-center sm:text-left">
                        Total: <?php echo e($siswaList->count()); ?> siswa
                    </div>
...
function absensiManager() {
    return {
        ...
        absensiData: <?php echo json_encode($existingAbsensi ? $existingAbsensi->mapWithKeys(function($item, $key) {
        ...
        async filterByDate(date) {
            ...
            try {
                const response = await fetch(`<?php echo e(route("siswa.ketua-kelas.absensi")); ?>?tanggal=${date}`, {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    window.history.pushState({}, '', `<?php echo e(route("siswa.ketua-kelas.absensi")); ?>?tanggal=${date}`);
                    this.showNotificationMessage('Data berhasil dimuat', 'success');
                } else {
                    this.showNotificationMessage('Gagal memuat data', 'error');
                    setTimeout(() => {
                        window.location.href = `<?php echo e(route("siswa.ketua-kelas.absensi")); ?>?tanggal=${date}`;
                    }, 2000);
                }
            } catch (error) {
...
<?php echo $__env->make('layouts.siswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\website-smk3\resources\views\siswa\ketua-kelas\absensi_backup.blade.php ENDPATH**/ ?>