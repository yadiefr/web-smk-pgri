...
class PendaftaranController extends Controller
{
    ...
    {
        $this->middleware('auth:pendaftar')->except(['index', 'landing', 'check', 'checkStatus', 'success']);
    }
    ...
    {
        // Halaman informasi PPDB publik - dapat diakses semua orang
        
        // Ambil pengaturan PPDB
        $is_ppdb_open = Settings::getValue('is_ppdb_open', 'true') === 'true';
        $ppdb_start_date = Settings::getValue('ppdb_start_date');
        $ppdb_end_date = Settings::getValue('ppdb_end_date');
        $tahun_ajaran = Settings::getValue('ppdb_year', date('Y') . '/' . (date('Y') + 1));
        
        // Siapkan periode pendaftaran jika ada
        $periode_pendaftaran = null;
        if ($ppdb_start_date && $ppdb_end_date) {
            $periode_pendaftaran = [
                'start' => $ppdb_start_date,
                'end' => $ppdb_end_date
            ];
        }
        
        // Ambil semua jurusan yang tersedia
        $jurusan_tersedia = Jurusan::all()->map(function($jurusan) {
            // Tambahkan data tambahan jika diperlukan
            $jurusan->kuota = $jurusan->kuota ?? 32; // Default kuota
            
            // Contoh prospek kerja (bisa diambil dari database atau config)
            $prospek_kerja = [
                'Staff Administrasi',
                'Operator Komputer', 
                'Teknisi IT',
            ...
            $jurusan->prospek_kerja = $prospek_kerja;
    ...
    public function formOrDashboard()
    {
        // Cek apakah PPDB sedang dibuka
        $is_ppdb_open = Settings::getValue('is_ppdb_open', 'false') === 'true';
        $ppdb_start_date = Settings::getValue('ppdb_start_date');
        ...
        // Jika sudah melengkapi data lengkap (ada pilihan jurusan), redirect ke dashboard
        if ($pendaftaran && $pendaftaran->pilihan_jurusan_1) {
            return redirect()->route('pendaftar.dashboard');
        }
