...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="grid grid-cols-1 gap-3">
                        <div class="flex justify-between border-b pb-2">
                            ...
                            <span class="font-semibold">{{ $referenceAbsensi->kelas->nama_kelas ?? 'Tidak diketahui' }}</span>
                        </div>
                        <div class="flex justify-between border-b pb-2">
                            <span class="font-medium text-gray-600">
                                <i class="fas fa-book mr-1"></i>Mata Pelajaran:
                            </span>
                            <span class="font-semibold">{{ $referenceAbsensi->mapel->nama ?? 'Tidak diketahui' }}</span>
                        </div>
                        <div class="flex justify-between border-b pb-2">
                            <span class="font-medium text-gray-600">
                                <i class="fas fa-calendar mr-1"></i>Tanggal:
                            </span>
                            <span class="font-semibold">{{ \Carbon\Carbon::parse($referenceAbsensi->tanggal)->format('d F Y') }}</span>
                        </div>
                        <div class="flex justify-between border-b pb-2">
                            <span class="font-medium text-gray-600">
                                <i class="fas fa-chalkboard-teacher mr-1"></i>Guru:
                            </span>
                            <span class="font-semibold">{{ $referenceAbsensi->guru->nama ?? 'Tidak diketahui' }}</span>
                        </div>
                        @if($referenceAbsensi->guru && $referenceAbsensi->guru->nip)
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-600">
                                <i class="fas fa-id-card mr-1"></i>NIP Guru:
                            </span>
                            <span class="text-gray-700">{{ $referenceAbsensi->guru->nip }}</span>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200 text-center">
                        <div class="text-lg font-semibold text-gray-800">Total: {{ $stats['total'] }} Siswa</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            @if(isset($absensi) && $absensi->count() > 0)
                <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p class="text-sm text-blue-700">
                        ...
                        Menampilkan {{ $absensi->count() }} siswa untuk sesi ini
                    </p>
                </div>
            @else
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $data->siswa->nama_lengkap ?? $data->siswa->nama ?? 'Tidak diketahui' }}</div>
                            </td>
                    ...
                    <div class="w-full bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                            ...
                            <div class="flex items-center flex-1 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                    <h3 class="text-sm font-semibold text-gray-900 break-words">
                                        {{ $data->siswa->nama_lengkap ?? $data->siswa->nama ?? 'Tidak diketahui' }}
                                    </h3>
                        ...
                        <div class="mt-3 w-full">
                            <div class="flex items-start w-full">
                                ...
                                <span class="text-xs text-gray-700 flex-1 break-words">{{ $data->keterangan }}</span>
                            </div>
                        </div>
                        @endif
                    </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <a href="{{ route('guru.absensi.edit', $absensi->first()->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg transition-colors text-center">
                <i class="fas fa-edit mr-2"></i>Edit
            </a>
            <a href="{{ route('guru.absensi.destroy', $absensi->first()->id) }}" onclick="return confirm('Apakah Anda yakin ingin menghapus data absensi ini?')" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition-colors text-center">
