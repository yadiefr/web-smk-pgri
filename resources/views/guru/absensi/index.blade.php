    ...
    <div class="bg-white rounded-lg shadow-lg mobile-spacing md:p-6">
        <div class="flex mobile-header md:justify-between md:items-center mobile-section-spacing md:mb-6">
            ...
            <div class="mobile-header-actions md:flex md:space-x-3">
                <a href="{{ route('guru.absensi.create') }}" class="action-btn bg-blue-600 hover:bg-blue-700 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center">
                    <i class="fas fa-plus mr-1 md:mr-2"></i>
                    <span class="hidden sm:inline">Tambah </span>Absensi
                </a>
                <a href="{{ route('guru.absensi.rekap') }}" class="action-btn bg-green-600 hover:bg-green-700 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center">
        ...
        <div class="mobile-section-spacing md:mb-8">
            <form method="GET" action="{{ route('guru.absensi.index') }}" class="space-y-3 md:space-y-4">
                <div class="grid mobile-filter-grid md:grid-cols-3 gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal_filter" value="{{ request('tanggal') }}" class="w-full px-3 py-2 md:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                <div class="flex mobile-filter-actions md:items-center md:space-x-2 pt-3 md:pt-4">
                    ...
                    <a href="{{ route('guru.absensi.index') }}" class="action-btn bg-gray-500 hover:bg-gray-600 text-white px-3 md:px-4 py-2 rounded-lg transition-colors flex items-center">
        ...
        @if(request()->hasAny(['kelas_id', 'mapel_id', 'tanggal']))
        <div class="mb-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-1 text-sm text-blue-700">
                            @if(request('kelas_id'))
                                @php $selectedKelas = $kelas->where('id', request('kelas_id'))->first(); @endphp
                                Menampilkan data untuk kelas: <strong>{{ $selectedKelas->nama_kelas ?? 'Kelas tidak ditemukan' }}</strong>
                                @if(request('mapel_id') || request('tanggal'))
                                    <br>
                                @endif
                            @endif
                            @if(request('mapel_id'))
                                @php $selectedMapel = $mapel->where('id', request('mapel_id'))->first(); @endphp
                                Mata pelajaran: <strong>{{ $selectedMapel->nama ?? 'Mata pelajaran tidak ditemukan' }}</strong>
                                @if(request('tanggal'))
                                    <br>
                                @endif
                            @endif
                    ...
                    <div class="ml-auto">
                        <a href="{{ route('guru.absensi.index') }}" class="inline-flex items-center px-3 py-1.5 border border-blue-300 text-xs font-medium rounded text-blue-700 bg-blue-100 hover:bg-blue-200 transition-colors">
        ...
        <div class="mobile-cards-container">
            @forelse($absensi_list as $absensi)
                <div class="mobile-absensi-card">
                    <div class="mobile-card-header">
                        <div>
                            <h3 class="mobile-card-title">{{ $absensi->kelas->nama_kelas ?? '-' }}</h3>
                            <p class="text-sm text-gray-600">{{ $absensi->mapel->nama ?? '-' }}</p>
                        </div>
                    ...
                    <div class="mobile-stats-row">
                        <div class="mobile-stat-item bg-blue-50 text-blue-700">
                            <div class="font-semibold">{{ $absensi->total_siswa }}</div>
                            <div>Total</div>
                        </div>
                        <div class="mobile-stat-item bg-green-50 text-green-700">
                            <div class="font-semibold">{{ $absensi->hadir }}</div>
                            <div>Hadir</div>
                        </div>
                        <div class="mobile-stat-item bg-red-50 text-red-700">
                            <div class="font-semibold">{{ $absensi->tidak_hadir }}</div>
                            <div>Tidak Hadir</div>
                        </div>
                    </div>

                    <div class="mobile-actions">
                        <a href="{{ route('guru.absensi.show', $absensi->id) }}" class="bg-blue-100 text-blue-700 hover:bg-blue-200">
                            <i class="fas fa-eye mr-1"></i> Detail
                        </a>
                        <a href="{{ route('guru.absensi.edit', $absensi->id) }}" class="bg-yellow-100 text-yellow-700 hover:bg-yellow-200">
                            <i class="fas fa-edit mr-1"></i> Edit
                        </a>
                        <button type="button" class="bg-red-100 text-red-700 hover:bg-red-200 delete-btn"
                                data-tanggal="{{ \Carbon\Carbon::parse($absensi->tanggal)->format('d/m/Y') }}"
                                data-kelas="{{ $absensi->kelas->nama_kelas ?? '-' }}"
                                data-mapel="{{ $absensi->mapel->nama ?? '-' }}"
                                data-absensi-id="{{ $absensi->id }}">
                            <i class="fas fa-trash mr-1"></i> Hapus
                        </button>
                        <!-- Hidden form for delete -->
                        <form action="{{ route('guru.absensi.destroy', $absensi->id) }}" method="POST" class="hidden delete-form" data-id="{{ $absensi->id }}">
                            @csrf
                            @method('DELETE')
                        </form>
                    </div>
            ...
            @empty
                <div class="text-center py-8 md:py-12">
                    ...
                    <a href="{{ route('guru.absensi.create') }}" class="action-btn bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors inline-flex items-center">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('guru.absensi.show', $absensi->id) }}" class="text-blue-600 hover:text-blue-900">
                                    <i class="fas fa-eye"></i> Detail
                                </a>
                                <a href="{{ route('guru.absensi.edit', $absensi->id) }}" class="text-yellow-600 hover:text-yellow-900">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('guru.absensi.destroy', $absensi->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                            <div class="flex flex-col items-center py-8">
                                ...
                                <p class="text-sm text-gray-400 mb-4">Silakan tambah data absensi atau ubah filter pencarian</p>
                                <a href="{{ route('guru.absensi.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors">
                                    <i class="fas fa-plus mr-2"></i> Tambah Absensi
                                </a>
                            </div>
                        </td>
...
<script>
    ...
    function filterKelasByMapel() {
        ...
        if (mapelId) {
            ...
            fetch(`{{ route('guru.absensi.kelas-by-mapel') }}?mapel_id=${mapelId}`)
            ...
            // Reset to all kelas
            fetch(`{{ route('guru.absensi.kelas-by-mapel') }}`)
                .then(response => response.json())
                .then(data => {
                    kelasSelect.innerHTML = '<option value="">Semua Kelas</option>';
                    data.forEach(kelas => {
