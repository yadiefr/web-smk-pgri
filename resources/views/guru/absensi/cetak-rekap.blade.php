...
<body>
    ...
    <div class="header">
        <h1>{{ strtoupper(setting('nama_sekolah', 'SMK PGRI CIKAMPEK')) }}</h1>
    ...
    <div class="info-section">
        <table class="info-table">
            ...
            <tr>
                <td>Periode</td>
                <td>: {{ \Carbon\Carbon::parse($tanggalAwal)->format('d/m/Y') }} s/d {{ \Carbon\Carbon::parse($tanggalAkhir)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td>Jumlah Hari Absensi</td>
                <td>: {{ count($periode) }} hari</td>
            </tr>
            <tr>
                <td>Tanggal Cetak</td>
    ...
    <table class="attendance-table">
        <thead>
            <tr>
                ...
                <th colspan="{{ count($periode) }}">Tanggal</th>
                <th colspan="4">Jumlah</th>
            </tr>
            <tr>
                @foreach($periode as $tgl)
                <th style="width: 25px;">{{ \Carbon\Carbon::parse($tgl)->format('d') }}</th>
                @endforeach
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $i + 1 }}</td>
                <td>{{ $s->nis }}</td>
                <td class="student-info">{{ $s->nama_lengkap }}</td>
                ...
                <td class="status-h">{{ isset($rekapData[$s->id]['hadir']) ? $rekapData[$s->id]['hadir'] : 0 }}</td>
                <td class="status-i">{{ isset($rekapData[$s->id]['izin']) ? $rekapData[$s->id]['izin'] : 0 }}</td>
                <td class="status-s">{{ isset($rekapData[$s->id]['sakit']) ? $rekapData[$s->id]['sakit'] : 0 }}</td>
                <td class="status-a">{{ isset($rekapData[$s->id]['alpha']) ? $rekapData[$s->id]['alpha'] : 0 }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer">
        ...
        <div class="signature">
            <p>{{ setting('alamat_kota', 'Cikampek') }}, {{ \Carbon\Carbon::now()->format('d F Y') }}<br>Guru Mata Pelajaran</p>
            <div class="signature-line">
