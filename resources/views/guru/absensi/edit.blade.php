...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <form id="absensi-form" action="{{ route('guru.absensi.update', $absensi->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
                ...
                <div class="sm:col-span-2 lg:col-span-1">
                    ...
                    <input type="date" name="tanggal" id="tanggal" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 text-sm"
                        value="{{ old('tanggal', $absensi->tanggal->format('Y-m-d')) }}" required readonly>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700">
                            <strong>Mode Edit:</strong> Anda sedang mengedit absensi untuk tanggal {{ \Carbon\Carbon::parse($absensi->tanggal)->format('d/m/Y') }}, 
                            kelas {{ $absensi->kelas->nama_kelas ?? 'N/A' }}, mata pelajaran {{ $absensi->mapel->nama ?? 'N/A' }}.
                ...
                @if(isset($allAbsensi))
                    <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <p class="text-sm text-blue-700">
                            ...
                            Ditemukan {{ $allAbsensi->count() }} siswa untuk sesi ini
                        </p>
                    </div>
                @else
                ...
                <div id="desktop-view" class="overflow-x-auto bg-white rounded-lg shadow">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="siswa-list-desktop" class="bg-white divide-y divide-gray-200">
                            @if(isset($allAbsensi) && $allAbsensi->count() > 0)
                                @foreach($allAbsensi as $index => $absensiItem)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $index + 1 }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $absensiItem->siswa->nis ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $absensiItem->siswa->nama_lengkap ?? $absensiItem->siswa->nama ?? 'Nama Tidak Ditemukan' }}</div>
                                        <input type="hidden" name="siswa_id[]" value="{{ $absensiItem->siswa_id }}">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex space-x-3">
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="status[{{ $index }}]" value="hadir" {{ $absensiItem->status == 'hadir' ? 'checked' : '' }} class="form-radio text-green-600 mr-1">
                                                <span class="text-sm">H</span>
                                            </label>
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="status[{{ $index }}]" value="izin" {{ $absensiItem->status == 'izin' ? 'checked' : '' }} class="form-radio text-blue-600 mr-1">
                                                <span class="text-sm">I</span>
                                            </label>
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="status[{{ $index }}]" value="sakit" {{ $absensiItem->status == 'sakit' ? 'checked' : '' }} class="form-radio text-yellow-600 mr-1">
                                                <span class="text-sm">S</span>
                                            </label>
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="status[{{ $index }}]" value="alpha" {{ $absensiItem->status == 'alpha' ? 'checked' : '' }} class="form-radio text-red-600 mr-1">
                                                <span class="text-sm">A</span>
                                            </label>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="text" name="keterangan[]" value="{{ $absensiItem->keterangan }}" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" placeholder="Keterangan">
                                    </td>
                                </tr>
                                @endforeach
                            @else
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                                ...
                                <div class="flex items-start mb-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-sm font-semibold text-gray-900 break-words">
                                            {{ $absensiItem->siswa->nama_lengkap ?? $absensiItem->siswa->nama ?? 'Nama Tidak Ditemukan' }}
                                        </h3>
                                        <p class="text-xs text-gray-500 mt-1">
                                            <i class="fas fa-id-badge mr-1"></i>
                                            NIS: {{ $absensiItem->siswa->nis ?? 'N/A' }}
                                        </p>
                                        <input type="hidden" name="siswa_id[]" value="{{ $absensiItem->siswa_id }}">
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="flex gap-2">
                                        <label class="flex items-center justify-center p-2 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $absensiItem->status == 'hadir' ? 'bg-green-50 border-green-200' : '' }} flex-1">
                                            <input type="radio" name="status[{{ $index }}]" value="hadir" {{ $absensiItem->status == 'hadir' ? 'checked' : '' }} class="mr-1">
                                            <span class="text-sm font-medium">H</span>
                                        </label>
                                        <label class="flex items-center justify-center p-2 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $absensiItem->status == 'izin' ? 'bg-blue-50 border-blue-200' : '' }} flex-1">
                                            <input type="radio" name="status[{{ $index }}]" value="izin" {{ $absensiItem->status == 'izin' ? 'checked' : '' }} class="mr-1">
                                            <span class="text-sm font-medium">I</span>
                                        </label>
                                        <label class="flex items-center justify-center p-2 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $absensiItem->status == 'sakit' ? 'bg-yellow-50 border-yellow-200' : '' }} flex-1">
                                            <input type="radio" name="status[{{ $index }}]" value="sakit" {{ $absensiItem->status == 'sakit' ? 'checked' : '' }} class="mr-1">
                                            <span class="text-sm font-medium">S</span>
                                        </label>
                                        <label class="flex items-center justify-center p-2 border rounded-lg cursor-pointer hover:bg-gray-50 {{ $absensiItem->status == 'alpha' ? 'bg-red-50 border-red-200' : '' }} flex-1">
                                            <input type="radio" name="status[{{ $index }}]" value="alpha" {{ $absensiItem->status == 'alpha' ? 'checked' : '' }} class="mr-1">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="keterangan[]" value="{{ $absensiItem->keterangan }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" placeholder="Tambahkan keterangan...">
                                </div>
                            </div>
                            @endforeach
                        @endif
            ...
            <div class="mt-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <a href="{{ route('guru.absensi.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-3 rounded-lg transition-colors text-center sm:text-left order-2 sm:order-1">
