...
@section('title', 'Edit Materi - SMK PGRI CIKAMPEK')
...
<div class="px-3 py-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <form action="{{ route('guru.materi.update-materi', $materi->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="p-6">
                <div class="grid lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="kelas_id" name="kelas_id" required
                                ...
                                @foreach($kelasList as $kelas)
                                    <option value="{{ $kelas->id }}" {{ (old('kelas_id', $materi->kelas_id) == $kelas->id) ? 'selected' : '' }}>
                                        {{ $kelas->nama_kelas }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="url" id="link_video" name="link_video" value="{{ old('link_video', $materi->link_video) }}"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="8" required
                                      ...
                                      placeholder="Jelaskan tentang materi pembelajaran ini...">{{ old('deskripsi', $materi->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-blue-700">{{ $materi->file_name ?? basename($materi->file_path) }}</span>
                        ...
                        <div>
                            <label for="file" class="block text-sm font-medium text-gray-700 mb-2">
                                {{ $materi->file_path ? 'Ganti File Materi (Opsional)' : 'File Materi (Opsional)' }}
                            </label>
