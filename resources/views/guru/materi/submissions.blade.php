...
@section('title', 'Jawaban Siswa - ' . $tugas->judul)
        ...
        <div class="flex mobile-header md:items-center md:justify-between">
            <div class="mobile-header-info">
                <h1 class="mobile-title md:text-2xl font-bold text-gray-800 flex items-center">
                    ...
                    Jawaban Siswa: {{ $tugas->judul }}
                </h1>
                <p class="mobile-subtitle md:text-base text-gray-600 mt-1">
                    {{ $tugas->kelas->nama_kelas ?? '-' }} • {{ $tugas->mapel->nama ?? '-' }}
                </p>
            </div>
            <div class="mobile-deadline md:text-right">
                ...
                <div class="text-sm md:text-lg font-semibold text-gray-800">
                    {{ $tugas->tanggal_deadline ? \Carbon\Carbon::parse($tugas->tanggal_deadline)->format('d M Y') : '-' }}
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="mobile-stat-card stat-card bg-white rounded-xl p-3 md:p-6 border border-gray-100 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3 md:ml-4">
                    ...
                    <h3 class="text-lg md:text-2xl font-bold text-gray-800">{{ $submissions->total() }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="mobile-stat-card stat-card bg-white rounded-xl p-3 md:p-6 border border-gray-100 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3 md:ml-4">
                    ...
                    <h3 class="text-lg md:text-2xl font-bold text-gray-800">{{ $siswaKelas->count() }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="mobile-stat-card stat-card bg-white rounded-xl p-3 md:p-6 border border-gray-100 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3 md:ml-4">
                    ...
                    <h3 class="text-lg md:text-2xl font-bold text-gray-800">{{ $submissions->total() + $siswaKelas->count() }}</h3>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="md:border-b md:border-gray-200">
            <nav class="mobile-tabs md:-mb-px md:flex md:space-x-8 md:px-6" aria-label="Tabs">
                <button onclick="showTab('submitted')" id="tab-submitted"
                    ...
                    <span class="hidden sm:inline">Sudah </span>Mengumpulkan ({{ $submissions->total() }})
                </button>
                <button onclick="showTab('not-submitted')" id="tab-not-submitted"
                    ...
                    <span class="hidden sm:inline">Belum </span>Mengumpulkan ({{ $siswaKelas->count() }})
                </button>
            </nav>
        </div>
                            ...
                            <div class="flex mobile-submission-header md:items-start md:justify-between">
                                <div class="mobile-student-info">
                                    <div class="mobile-student-avatar w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-gray-200 student-avatar mr-3 md:mr-4">
                                        @if($submission->siswa->foto && Storage::disk('public')->exists($submission->siswa->foto))
                                            <img src="{{ asset('storage/' . $submission->siswa->foto) }}"
                                                 alt="Foto {{ $submission->siswa->nama_lengkap }}"
                                                 class="w-full h-full object-cover">
                                        @else
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode($submission->siswa->nama_lengkap) }}&background=3b82f6&color=ffffff&size=48"
                                                 alt="Foto {{ $submission->siswa->nama_lengkap }}"
                                                 class="w-full h-full object-cover">
                                        @endif
                                    </div>
                                    <div class="mobile-student-details flex-1">
                                        <h4 class="mobile-student-name md:text-lg font-semibold text-gray-900">{{ $submission->siswa->nama_lengkap }}</h4>
                                        <p class="text-xs md:text-sm text-gray-600">
                                            {{ $submission->siswa->nis ?? $submission->siswa->nisn }}
                                        </p>
                                        <div class="mt-2 flex mobile-submission-meta md:items-center text-sm text-gray-500 md:space-x-4">
                                            <span>
                                                <i class="fas fa-clock mr-1"></i>
                                                <span class="hidden sm:inline">Dikumpulkan: </span>{{ $submission->created_at->format('d M Y H:i') }}
                                            </span>
                                        ...
                                        @if($submission->catatan)
                                            <div class="mobile-notes mt-3 p-2 md:p-3 bg-white rounded border-l-4 border-blue-500">
                                                ...
                                                <p class="text-xs md:text-sm text-gray-600 mt-1">{{ $submission->catatan }}</p>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <div class="mobile-student-avatar w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-gray-200 student-avatar mr-3 md:mr-4">
                                        @if($siswa->foto && Storage::disk('public')->exists($siswa->foto))
                                            <img src="{{ asset('storage/' . $siswa->foto) }}"
                                                 ...
                                                 class="w-full h-full object-cover">
                                        @else
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode($siswa->nama_lengkap) }}&background=ef4444&color=ffffff&size=48"
                                                 alt="Foto {{ $siswa->nama_lengkap }}"
                                                 class="w-full h-full object-cover">
                                        @endif
                                    </div>
                                    <div class="mobile-student-details flex-1">
                                        <h4 class="mobile-student-name md:text-lg font-semibold text-gray-900">{{ $siswa->nama_lengkap }}</h4>
                                        <p class="text-xs md:text-sm text-gray-600">
