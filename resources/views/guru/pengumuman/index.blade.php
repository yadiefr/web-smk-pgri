                    ...
                    <div class="p-6">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <h2 class="text-lg font-semibold text-gray-800">
                                        <a href="{{ route('guru.pengumuman.show', $p->id) }}" class="hover:text-blue-600">
                                            {{ $p->judul }}
                                        </a>
                                    </h2>
                                    <p class="text-sm text-gray-500">
                                        {{ \Carbon\Carbon::parse($p->tanggal_mulai)->format('d F Y') }}
                                    </p>
                                </div>
                            </div>
                            <span class="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">
                                {{ $p->target_role == 'all' ? 'Semua' : ucfirst($p->target_role) }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center text-xs text-gray-500">
                                ...
                                <span>Berlaku sampai {{ \Carbon\Carbon::parse($p->tanggal_selesai)->format('d M Y') }}</span>
                                @endif
                            </div>
                            <a href="{{ route('guru.pengumuman.show', $p->id) }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-700">
                                Baca selengkapnya
