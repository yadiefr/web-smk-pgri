...
@section('page-title', 'Tunggakan Pembayaran')
...
<div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-red-500 to-red-600 p-6 rounded-xl text-white shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">{{ $tunggakan->total() ?? 0 }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-orange-500 to-orange-600 p-6 rounded-xl text-white shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">Rp {{ number_format(($tunggakan->sum('nominal') ?? 0), 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 p-6 rounded-xl text-white shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">{{ $tunggakan->where('tanggal_jatuh_tempo', '<', now()->subDays(30))->count() ?? 0 }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 p-6 rounded-xl text-white shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">{{ $tunggakan->where('tanggal_jatuh_tempo', '<', now()->subDays(60))->count() ?? 0 }}</p>
                </div>
    ...
    <!-- Priority Alert -->
    @if($tunggakan->where('tanggal_jatuh_tempo', '<', now()->subDays(60))->count() > 0)
        ...
        <div class="flex items-center">
            ...
            <div class="ml-3">
                ...
                <div class="mt-2 text-sm text-red-700">
                    <p>Ada <strong>{{ $tunggakan->where('tanggal_jatuh_tempo', '<', now()->subDays(60))->count() }}</strong> siswa dengan tunggakan lebih dari 60 hari. Segera lakukan tindakan follow up.</p>
                </div>
                <div class="mt-4">
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-600">
                    <span>Menampilkan {{ $tunggakan->firstItem() ?? 0 }} - {{ $tunggakan->lastItem() ?? 0 }} dari {{ $tunggakan->total() ?? 0 }} data</span>
                </div>
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ ($tunggakan->currentPage() - 1) * $tunggakan->perPage() + $loop->iteration }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ \Carbon\Carbon::parse($item->tanggal_jatuh_tempo ?? now()->subDays(30))->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                            ...
                            <div class="space-y-1">
                                <div class="text-sm font-semibold text-red-600">{{ $lamaHari }} hari</div>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('tata-usaha.keuangan.tagihan') }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-200">
                                        <i class="fas fa-file-invoice mr-2"></i>
                                        Lihat Semua Tagihan
                                    </a>
                                    <a href="{{ route('tata-usaha.keuangan.pembayaran') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors duration-200">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const kritisCount = {{ $tunggakan->where('tanggal_jatuh_tempo', '<', now()->subDays(60))->count() ?? 0 }};
