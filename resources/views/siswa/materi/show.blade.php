        ...
        <div class="mb-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <nav class="flex items-center space-x-2 text-sm text-gray-500">
                    <a href="{{ route('siswa.dashboard') }}" class="hover:text-blue-600 transition-colors">
                        <i class="fas fa-home mr-1"></i>Dashboard
                    </a>
                    <i class="fas fa-chevron-right text-xs"></i>
                    <a href="{{ route('siswa.materi.index') }}" class="hover:text-blue-600 transition-colors">Materi & Tugas</a>
                    <i class="fas fa-chevron-right text-xs"></i>
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center justify-between mb-3">
                                <div class="inline-flex items-center bg-white/20 backdrop-blur-sm rounded-full px-3 py-1">
                                    ...
                                    <span class="font-semibold text-sm">{{ $materi->mataPelajaran ? $materi->mataPelajaran->nama : 'Mata Pelajaran' }}</span>
                                </div>
                                <div class="text-right">
                                    <div class="text-xs opacity-90 mb-1">Diunggah</div>
                                    <div class="font-medium text-sm">{{ $materi->created_at->isoFormat('D MMM Y') }}</div>
                                </div>
                            </div>
                            
                            <h1 class="text-2xl font-bold mb-3 leading-tight">{{ $materi->judul }}</h1>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <div class="font-semibold text-sm">{{ $materi->guru ? $materi->guru->nama_lengkap : 'Guru' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                                <div class="flex items-center justify-between">
                                    ...
                                    <a href="{{ route('siswa.materi.download', $materi->id) }}" 
                    ...
                    <div class="p-4 space-y-4">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <div class="font-semibold text-sm">{{ $materi->created_at->isoFormat('D MMMM Y') }}</div>
                            </div>
                        </div>
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <div class="font-semibold text-sm">{{ $materi->created_at->isoFormat('HH:mm') }} WIB</div>
                            </div>
                        </div>
                        @if($materi->file_path)
                        <div class="flex items-center">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="border border-gray-200 rounded-lg p-3 hover:border-blue-300 hover:shadow-md transition-all duration-200">
                                <div class="flex items-start justify-between mb-2">
                                    <h4 class="font-semibold text-gray-800 text-sm leading-tight">{{ Str::limit($tugas->judul, 40) }}</h4>
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ml-2 {{ $tugas->isExpired() ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                                        <i class="fas fa-{{ $tugas->isExpired() ? 'times-circle' : 'check-circle' }} mr-1"></i>
                                        {{ $tugas->isExpired() ? 'Berakhir' : 'Aktif' }}
                                    </span>
                                </div>
                                
                                <p class="text-gray-600 text-xs mb-3 leading-relaxed">{{ Str::limit($tugas->deskripsi, 80) }}</p>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center text-xs text-gray-500">
                                        ...
                                        <span>
                                            @if($tugas->deadline)
                                                {{ $tugas->deadline->isoFormat('D MMM Y') }}
                                            @elseif($tugas->tanggal_deadline)
                                                {{ \Carbon\Carbon::parse($tugas->tanggal_deadline)->isoFormat('D MMM Y') }}
                                            @else
                                                No deadline
                                            @endif
                                        </span>
