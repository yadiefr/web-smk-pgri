...
@section('title', 'Daftar Kas Keluar - Bendahara')
...
<div class="h-full bg-gray-50 p-3 sm:p-4 lg:p-6">
    <div class="w-full h-full">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex-1">
                    ...
                    <nav class="text-xs sm:text-sm text-gray-600">
                        ...
                        <a href="{{ route('siswa.bendahara.dashboard') }}" class="hover:text-blue-600">Bendahara</a>
                        <span class="mx-1 sm:mx-2">></span>
                        <span class="text-blue-600">Kas Keluar</span>
                    </nav>
                </div>
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <a href="{{ route('siswa.bendahara.input-kas-keluar') }}"
            ...
            <div class="bg-white p-4 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold {{ $saldoKas >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            Rp {{ number_format($saldoKas, 0, ',', '.') }}
                        </p>
                    </div>
                    <div class="p-3 rounded-full {{ $saldoKas >= 0 ? 'bg-green-100' : 'bg-red-100' }}">
                        <i class="fas fa-wallet {{ $saldoKas >= 0 ? 'text-green-600' : 'text-red-600' }} text-xl"></i>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-600">
                            Rp {{ number_format($totalKasKeluarBulan, 0, ',', '.') }}
                        </p>
                    </div>
            ...
            <div class="bg-white p-4 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-600">{{ $jumlahTransaksiBulan }}</p>
        ...
        <div class="bg-white p-4 rounded-lg shadow mb-6">
            <form method="GET" action="{{ route('siswa.bendahara.kas-keluar') }}" class="flex flex-wrap gap-4">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">
                    Data Kas Keluar - {{ \Carbon\Carbon::createFromDate($tahun, $bulan, 1)->format('F Y') }}
                </h3>
            </div>

            @if($kasKeluarBulanIni->count() > 0)
                <!-- Desktop Table -->
                <!-- Desktop Table -->
                <div class="desktop-table hidden lg:block overflow-x-auto" style="display: none !important;">
                    <table class="w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($kasKeluarBulanIni as $kas)
                    ...
                    @foreach($kasKeluarBulanIni as $kas)
                        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex-1">
                                    <h4 class="text-sm font-medium text-gray-900">{{ $kas->keterangan }}</h4>
                                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($kas->tanggal)->format('d M Y') }}</p>
                                    <p class="text-xs text-gray-600 mt-1">
                                        <span class="font-medium">Input oleh:</span> {{ $kas->siswa->nama_lengkap ?? 'System' }}
                                    </p>
                                </div>
                            ...
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-600">{{ $kas->siswa->nama_lengkap ?? 'System' }}</span>
                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">
                                    Kas Keluar
                                </span>
                            </div>
            ...
            @else
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-600 mb-4">
                        Belum ada transaksi kas keluar untuk {{ \Carbon\Carbon::createFromDate($tahun, $bulan, 1)->format('F Y') }}
                    </p>
                    <a href="{{ route('siswa.bendahara.input-kas-keluar') }}" 
