...
<div class="w-full h-full" x-data="dashboardBendahara">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">{{ $kelas->nama_kelas }} - {{ $kelas->jurusan->nama_jurusan }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 hover:shadow-md transition-shadow">
            <div class="flex flex-col lg:flex-row lg:items-center">
                ...
                <div class="lg:ml-4 text-center lg:text-left">
                    ...
                    <p class="text-sm sm:text-lg lg:text-2xl font-bold text-gray-800">Rp {{ number_format($totalKasMasuk, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 hover:shadow-md transition-shadow">
            <div class="flex flex-col lg:flex-row lg:items-center">
                ...
                <div class="lg:ml-4 text-center lg:text-left">
                    ...
                    <p class="text-sm sm:text-lg lg:text-2xl font-bold text-gray-800">Rp {{ number_format($totalKasKeluar, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 hover:shadow-md transition-shadow">
            <div class="flex flex-col lg:flex-row lg:items-center">
                <div class="p-2 sm:p-3 rounded-full {{ $saldoKas >= 0 ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600' }} mb-2 lg:mb-0 self-center lg:self-auto">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="lg:ml-4 text-center lg:text-left">
                    ...
                    <p class="text-sm sm:text-lg lg:text-2xl font-bold {{ $saldoKas >= 0 ? 'text-blue-600' : 'text-red-600' }}">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 hover:shadow-md transition-shadow">
            <div class="flex flex-col lg:flex-row lg:items-center">
                ...
                <div class="lg:ml-4 text-center lg:text-left">
                    ...
                    <p class="text-sm sm:text-lg lg:text-2xl font-bold text-gray-800">{{ $jumlahTransaksi }}</p>
    ...
    <div class="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <button class="w-full bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 p-3 sm:p-4 rounded-lg text-center transition-all duration-200 shadow-sm hover:shadow-md min-h-[100px] sm:min-h-[120px] flex flex-col justify-center">
                <a href="{{ route('siswa.bendahara.export-kas', ['format' => 'excel', 'periode' => 'bulan']) }}"</a>
                <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-sm text-gray-600">
                Total: <span class="font-semibold">{{ $transaksiKasBulanIni->count() }}</span> transaksi
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($transaksiKasBulanIni->take(10) as $transaksi)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            @if(isset($transaksi['tanggal_terakhir']))
                                {{ Carbon\Carbon::parse($transaksi['tanggal_terakhir'])->format('d/m/Y') }}
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">
                            @if(isset($transaksi['siswa']))
                                <div class="font-medium">{{ $transaksi['siswa']['nama_lengkap'] }}</div>
                                <div class="text-xs text-gray-500">{{ $transaksi['siswa']['nis'] }}</div>
                            @else
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">
                            ...
                            @if(isset($transaksi['siswa']) && isset($transaksi['jumlah_transaksi']) && $transaksi['jumlah_transaksi'] > 1)
                                <div class="text-xs text-gray-500 mt-1">{{ $transaksi['jumlah_transaksi'] }} kali pembayaran</div>
                            @elseif(!isset($transaksi['siswa']))
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="font-semibold {{ $transaksi['tipe'] == 'masuk' ? 'text-green-600' : 'text-red-600' }}">
                                {{ $transaksi['tipe'] == 'masuk' ? '+' : '-' }} Rp {{ number_format($transaksi['total_nominal'] ?? $transaksi['nominal'], 0, ',', '.') }}
                            </span>
                        </td>
                    </tr>
                    @empty
        ...
        @if($transaksiKasBulanIni->count() > 10)
        <div class="mt-4 text-center" x-data="{ showAll: false }">
            <button @click="showAll = !showAll" 
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200">
                <span x-show="!showAll">Lihat Semua Transaksi ({{ $transaksiKasBulanIni->count() }})</span>
            ...
            <div x-show="showAll" x-transition class="mt-4 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($transaksiKasBulanIni->skip(10) as $transaksi)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if(isset($transaksi['tanggal_terakhir']))
                                    {{ Carbon\Carbon::parse($transaksi['tanggal_terakhir'])->format('d/m/Y') }}
...
document.addEventListener('alpine:init', () => {
    Alpine.data('dashboardBendahara', () => ({
        ...
        tambahKasMasuk() {
            window.location.href = '{{ route("siswa.bendahara.kas-masuk") }}';
        },
        
        tambahKasKeluar() {
            window.location.href = '{{ route("siswa.bendahara.kas-keluar") }}';
        },
        
        lihatLaporan() {
            window.location.href = '{{ route('siswa.bendahara.laporan-kas') }}';
        }
    }));
});
</script>
