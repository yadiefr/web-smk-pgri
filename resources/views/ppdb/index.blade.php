...
@section('title', 'PPDB SMK PGRI CIKAMPEK')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
    
    <!-- Countdown Timer Section (if PPDB is open) -->
    @if(($is_ppdb_open ?? true) && isset($periode_pendaftaran))
        ...
        <div class="container mx-auto px-4 py-16">
            <div class="text-center">
                ...
                <p class="text-xl mb-8 max-w-3xl mx-auto leading-relaxed">
                    Tahun Ajaran {{ $tahun_ajaran ?? date('Y') . '/' . (date('Y') + 1) }}
                </p>
                
                @if($is_ppdb_open ?? true)
                ...
                <div class="text-center">
                    ...
                    <div class="bg-green-50 border-2 border-green-200 rounded-lg p-4">
                        <p class="text-2xl font-bold text-green-600">
                            {{ \Carbon\Carbon::parse($periode_pendaftaran['start'])->format('d F Y') }}
                        </p>
                    </div>
                </div>
                <div class="text-center">
                    ...
                    <div class="bg-red-50 border-2 border-red-200 rounded-lg p-4">
                        <p class="text-2xl font-bold text-red-600">
                            {{ \Carbon\Carbon::parse($periode_pendaftaran['end'])->format('d F Y') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    @if(isset($jurusan_tersedia) && count($jurusan_tersedia) > 0)
    <div class="container mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-8">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($jurusan_tersedia as $jurusan)
                    <div class="bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-all duration-300">
                        <div class="text-center mb-4">
                            ...
                            <h4 class="text-xl font-bold text-gray-800 mb-2">{{ $jurusan->nama }}</h4>
                            @if($jurusan->deskripsi)
                                <p class="text-gray-600 text-sm leading-relaxed">{{ $jurusan->deskripsi }}</p>
                            @endif
                        </div>
                        
                        @if(isset($jurusan->kuota))
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                            <div class="flex justify-between items-center">
                                <span class="text-sm font-medium text-gray-700">Kuota Tersedia:</span>
                                <span class="text-lg font-bold text-blue-600">{{ $jurusan->kuota }} siswa</span>
                            </div>
                        </div>
                        @endif

                        @if(isset($jurusan->prospek_kerja) && count($jurusan->prospek_kerja) > 0)
                        <div class="mt-4">
                            <h5 class="text-sm font-semibold text-gray-700 mb-2">Prospek Kerja:</h5>
                            <ul class="text-xs text-gray-600 space-y-1">
                                @foreach(array_slice($jurusan->prospek_kerja, 0, 3) as $prospek)
                                <li class="flex items-start">
                ...
                <div class="space-y-4">
                    <div class="border border-gray-200 rounded-lg">
                        ...
                        <div class="faq-content hidden px-6 py-4 bg-white" id="faq1">
                            <p class="text-gray-600">Pendaftaran PPDB biasanya dibuka pada bulan Juni hingga Juli. Untuk tahun ajaran {{ $tahun_ajaran ?? '2024/2025' }}, pendaftaran akan dibuka sesuai kalender akademik yang telah ditetapkan. Pantau terus website resmi sekolah untuk informasi terbaru.</p>
                        </div>
                    </div>
...
function updateCountdown() {
    const endDate = new Date('{{ $periode_pendaftaran["end"] ?? "2024-07-31" }} 23:59:59').getTime();
