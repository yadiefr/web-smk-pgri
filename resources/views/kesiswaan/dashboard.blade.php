...
<div class="w-full px-3 py-4">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-blue-100">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="text-right">
                <div class="text-sm text-gray-500">{{ now()->locale('id')->isoFormat('dddd, D MMMM Y') }}</div>
        ...
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 border border-blue-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($totalSiswa) }}</h3>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($totalKelas) }}</h3>
                </div>
                <div class="bg-white p-3 rounded-lg shadow-sm">
        ...
        <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-4 border border-emerald-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($siswaAktif) }}</h3>
                </div>
                <div class="bg-white p-3 rounded-lg shadow-sm">
        ...
        <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-4 border border-red-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($siswaNonaktif) }}</h3>
                </div>
                <div class="bg-white p-3 rounded-lg shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div class="p-6">
                @if(!empty($absensiStats))
                    <div class="space-y-4">
                        @foreach($absensiStats as $status => $total)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <div class="w-4 h-4 rounded-full mr-3 
                                        {{ $status == 'hadir' ? 'bg-green-500' : '' }}
                                        {{ $status == 'izin' ? 'bg-yellow-500' : '' }}
                                        {{ $status == 'sakit' ? 'bg-blue-500' : '' }}
                                        {{ $status == 'alpa' ? 'bg-red-500' : '' }}"></div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div class="p-6">
                <div class="space-y-3 max-h-64 overflow-y-auto">
                    @forelse($kelasStats as $kelas)
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div>
                                <span class="font-medium text-gray-900">{{ $kelas->nama_kelas }}</span>
                                @if($kelas->jurusan)
                                    <span class="text-sm text-gray-500">- {{ $kelas->jurusan->nama_jurusan ?? '' }}</span>
                                @endif
                            </div>
                            <div class="text-right">
                                <span class="text-lg font-bold text-blue-600">{{ $kelas->siswa_count }}</span>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
                    <div class="flex items-center">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center mr-4
                            {{ $absensi->status == 'hadir' ? 'bg-green-100 text-green-600' : '' }}
                            {{ $absensi->status == 'izin' ? 'bg-yellow-100 text-yellow-600' : '' }}
                            {{ $absensi->status == 'sakit' ? 'bg-blue-100 text-blue-600' : '' }}
                            {{ $absensi->status == 'alpa' ? 'bg-red-100 text-red-600' : '' }}">
                            <i class="fas 
                                {{ $absensi->status == 'hadir' ? 'fa-check' : '' }}
                                {{ $absensi->status == 'izin' ? 'fa-info' : '' }}
                                {{ $absensi->status == 'sakit' ? 'fa-plus' : '' }}
                                {{ $absensi->status == 'alpa' ? 'fa-times' : '' }}"></i>
                        </div>
                        <div>
                            <p class="font-medium text-gray-900">{{ $absensi->siswa->nama_lengkap ?? 'Nama tidak tersedia' }}</p>
                            <p class="text-sm text-gray-500">
                                {{ $absensi->kelas->nama_kelas ?? 'Kelas tidak tersedia' }} • 
                                <span class="capitalize">{{ $absensi->status }}</span>
                            </p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-medium text-gray-900">{{ $absensi->tanggal ? \Carbon\Carbon::parse($absensi->tanggal)->format('d/m/Y') : '-' }}</p>
                        <p class="text-xs text-gray-500">{{ $absensi->created_at ? $absensi->created_at->diffForHumans() : '-' }}</p>
                    </div>
                </div>
            @empty
                <div class="text-center py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <a href="{{ route('kesiswaan.siswa.index') }}" class="flex items-center p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                ...
                <a href="{{ route('kesiswaan.absensi.index') }}" class="flex items-center p-3 bg-green-50 rounded-lg hover:bg-green-100 transition-colors">
                ...
                <a href="{{ route('kesiswaan.kegiatan.index') }}" class="flex items-center p-3 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors">
                ...
                <a href="{{ route('kesiswaan.laporan.index') }}" class="flex items-center p-3 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors">
