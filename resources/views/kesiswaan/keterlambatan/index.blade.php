...
@section('title', 'Data Keterlambatan Siswa')
...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('kesiswaan.keterlambatan.rekap') }}" class="bg-gradient-to-r from-green-500 to-green-600 text-white px-4 py-2 rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-300 flex items-center gap-2 shadow-sm">
                    <i class="fas fa-file-excel"></i>
                    <span>Export</span>
                </a>
                <a href="{{ route('kesiswaan.keterlambatan.create') }}" class="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2 rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-300 flex items-center gap-2 shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm p-3 sm:p-4 mb-4 border border-gray-100">
        <form method="GET" class="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <div class="flex gap-1 sm:gap-2">
                ...
                <a href="{{ route('kesiswaan.keterlambatan.index') }}" class="bg-gray-500 text-white px-2.5 py-1.5 rounded-lg hover:bg-gray-600 transition-colors flex items-center justify-center">
    ...
    <!-- Filter Info -->
    @if($filterInfo['is_default_filter'])
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                <p class="text-sm text-blue-700">
                    ...
                    ({{ \Carbon\Carbon::parse($filterInfo['tanggal_mulai'])->format('d M Y') }} - {{ \Carbon\Carbon::parse($filterInfo['tanggal_akhir'])->format('d M Y') }})
                    <br>
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                <p class="text-sm text-green-700">
                    ...
                    @endif
                    @if(request('kelas_id'))
                        | Kelas: {{ $kelas->where('id', request('kelas_id'))->first()->nama_kelas ?? 'N/A' }}
                    @endif
                </p>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($keterlambatanBatch as $batch)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-2 py-2 sm:px-6 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="font-medium">{{ $batch->tanggal->format('d/m/Y') }}</span>
                                    <span class="text-xs text-gray-500 hidden sm:block">{{ $batch->created_at->format('H:i') }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-2 py-2 sm:px-6 sm:py-4 whitespace-nowrap text-center">
                                @if($batch->status_umum == 'belum_ditindak')
                            ...
                            <td class="px-2 py-2 sm:px-6 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-center">
                                <div class="flex items-center justify-center space-x-1 sm:space-x-2">
                                    <button onclick="toggleBatchDetail({{ $loop->iteration }})" 
                                            class="text-blue-600 hover:text-blue-900 transition-colors p-1" 
                                            title="Detail">
                                        <i class="fas fa-chevron-down text-xs" id="toggle-icon-{{ $loop->iteration }}"></i>
                                    </button>
                                    <a href="{{ route('kesiswaan.keterlambatan.editBatch', [$batch->tanggal->format('Y-m-d'), $batch->petugas->id ?? 'null', $batch->created_at->format('Y-m-d H:i:s')]) }}" 
                                       class="text-indigo-600 hover:text-indigo-900 transition-colors p-1" 
                                       title="Edit">
                                        <i class="fas fa-edit text-xs"></i>
                                    </a>
                                    <button onclick="confirmDeleteBatch('{{ $batch->tanggal->format('Y-m-d') }}', '{{ $batch->petugas->id ?? 'null' }}', '{{ $batch->created_at->format('Y-m-d H:i:s') }}')" 
                        ...
                        <tr id="batch-detail-{{ $loop->iteration }}" class="hidden bg-blue-50">
                            <td colspan="5" class="px-2 py-2 sm:px-6 sm:py-4">
                                <div class="space-y-2">
                                    <h4 class="text-sm font-medium text-gray-900 mb-2">Detail Siswa ({{ $batch->jumlah_siswa }} orang):</h4>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
                                        @foreach($batch->siswa_list as $siswa)
                                            ...
                                            <div class="flex items-center justify-between mb-1 sm:mb-2">
                                                <div class="flex items-center flex-1 min-w-0">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        <div class="text-xs sm:text-sm font-medium text-gray-900 truncate">{{ $siswa['nama'] }}</div>
                                                        <div class="text-xs text-gray-500">{{ $siswa['nis'] }} • {{ $siswa['kelas'] }}</div>
                                                    </div>
                                                </div>
                                                <div class="text-right flex-shrink-0 ml-2">
                                                    <div class="text-xs font-medium text-orange-600">{{ $siswa['jam_terlambat'] }}</div>
                                                    <a href="{{ route('kesiswaan.keterlambatan.show', $siswa['id']) }}" 
                                                       class="text-xs text-blue-600 hover:text-blue-800">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="text-xs text-gray-600 truncate" title="{{ $siswa['alasan_terlambat'] }}">
                                                {{ $siswa['alasan_terlambat'] }}
                                            </div>
                                        </div>
                                        @endforeach
                                    </div>
...
function confirmDelete(id) {
    ...
    form.action = `{{ route('kesiswaan.keterlambatan.index') }}/${id}`;
...
function confirmDeleteBatch(tanggal, petugasId, createdAt) {
    ...
    const baseUrl = '{{ url("kesiswaan/keterlambatan") }}';
