...
@section('title', 'Detail Keterlambatan')
...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <a href="{{ route('kesiswaan.keterlambatan.edit', $keterlambatan->id) }}" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center mb-6">
                ...
                <div class="ml-4">
                    <h2 class="text-xl font-bold text-gray-900">{{ $keterlambatan->siswa->nama_lengkap }}</h2>
                    <p class="text-gray-600">NIS: {{ $keterlambatan->siswa->nis }}</p>
                    <p class="text-gray-600">Kelas: {{ $keterlambatan->kelas->nama_kelas }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex items-center text-gray-900">
                        ...
                        {{ $keterlambatan->tanggal->format('d F Y') }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center text-gray-900">
                        ...
                        <span class="font-semibold">{{ $keterlambatan->jam_terlambat_format }}</span>
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center">
                        @if($keterlambatan->status == 'belum_ditindak')
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">
                                <i class="fas fa-exclamation-circle mr-1"></i>
                                Belum Ditindak
                            </span>
                        @elseif($keterlambatan->status == 'sudah_ditindak')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span class="font-semibold text-orange-600">{{ $statistikSiswa['bulan_ini'] ?? 0 }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600">Total Keterlambatan Keseluruhan</span>
                    <span class="font-semibold text-red-600">{{ $statistikSiswa['total'] ?? 0 }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm text-gray-600">Dicatat Oleh</span>
                    <span class="font-semibold text-gray-900">{{ $keterlambatan->petugas->name ?? 'System' }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-gray-800">{{ $keterlambatan->alasan_terlambat ?: 'Tidak ada alasan yang dicatat' }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                @if($keterlambatan->sanksi)
                    <p class="text-gray-800">{{ $keterlambatan->sanksi }}</p>
                @else
    ...
    <!-- Catatan Petugas -->
    @if($keterlambatan->catatan_petugas)
    <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-400">
            <p class="text-gray-800">{{ $keterlambatan->catatan_petugas }}</p>
        </div>
    </div>
    @endif
    ...
    <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
                ...
                <p class="font-medium text-gray-900">{{ $keterlambatan->created_at->format('d/m/Y H:i') }}</p>
            </div>
            <div>
                <span class="text-gray-500">Terakhir diperbarui:</span>
                <p class="font-medium text-gray-900">{{ $keterlambatan->updated_at->format('d/m/Y H:i') }}</p>
            </div>
            <div>
                <span class="text-gray-500">ID Record:</span>
                <p class="font-medium text-gray-900">#{{ $keterlambatan->id }}</p>
            </div>
        </div>
    </div>
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-sm mx-4">
        ...
        <div class="flex justify-end space-x-3">
            ...
            <form action="{{ route('kesiswaan.keterlambatan.destroy', $keterlambatan->id) }}" method="POST" class="inline">
                @csrf
                @method('DELETE')
