...
@section('title', 'Rekap Keterlambatan Siswa')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-4">
        <div class="p-4">
            <form method="GET" action="{{ route('kesiswaan.keterlambatan.rekap') }}" id="filterForm">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-center text-gray-900">{{ $loop->iteration }}</td>
                            <td class="px-4 py-3 text-gray-900">{{ \Carbon\Carbon::parse($data->tanggal)->format('d/m/Y') }}</td>
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $data->siswa->nama_lengkap }}</div>
                                <div class="text-xs text-gray-500 md:hidden">
                                    {{ $data->siswa->nis }} | {{ $data->kelas->nama_kelas }}
                                </div>
                                @if($data->catatan_petugas)
                                <div class="text-xs text-blue-600 md:hidden mt-1">
                                    <i class="fas fa-info-circle"></i> {{ $data->catatan_petugas }}
                                </div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-900 hidden md:table-cell">{{ $data->kelas->nama_kelas }}</td>
                            ...
                            <td class="px-4 py-3 text-gray-900 hidden md:table-cell">{{ $data->petugas->name ?? '-' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
...
<div id="exportModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form id="exportForm" method="POST" action="{{ route('kesiswaan.keterlambatan.export') }}">
                @csrf
                <div class="mb-4">
