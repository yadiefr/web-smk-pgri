...
<head>
    ...
    <style type="text/css">
        ...
        .col-tanggal { width: 100px !important; max-width: 100px; min-width: 100px; }
        ...
        table.sheet0 col.col1 { width: 100px }
        table.sheet0 col.col2 { width: 240px }
        table.sheet0 col.col3 { width: 80px }
        table.sheet0 col.col4 { width: 60px }
        table.sheet0 col.col5 { width: 140px }
    </style>
</head>
<body>
    <table border="0" cellpadding="0" cellspacing="0" class="gridlines sheet0">
        ...
        <col class="col-tanggal col1" width="100" style="width: 100px;">
        <col class="col-nama col2" width="240" style="width: 240px;">
        <col class="col-kelas col3" width="80" style="width: 80px;">
        <col class="col-jam col4" width="60" style="width: 60px;">
        <col class="col-alasan col5" width="140" style="width: 140px;">
        
        <!-- Header Section -->
        <tr>
            <td colspan="6" class="title" style="text-align: center; padding: 10px 0;">DAFTAR SISWA DATANG TERLAMBAT</td>
        </tr>
        <tr>
            <td colspan="6" class="title" style="text-align: center; padding: 10px 0;">SMK PGRI CIKAMPEK</td>
        </tr>
        <tr>
            <td colspan="6" class="subtitle" style="text-align: center; padding: 10px 0;">TAHUN AJARAN {{ date('Y') }}-{{ date('Y') + 1 }}</td>
        </tr>
        <tr>
            <td colspan="6">&nbsp;</td>
        </tr>
        ...
        <tr>
            <td>&nbsp;</td>
            <td class="info-label" colspan="5" style="text-align: left; padding: 4px;">PERIODE : {{ \Carbon\Carbon::parse($tanggalMulai)->locale('id')->translatedFormat('d F Y') }} s/d {{ \Carbon\Carbon::parse($tanggalAkhir)->locale('id')->translatedFormat('d F Y') }}</td>
        </tr>
        <tr>
            <td colspan="6">&nbsp;</td>
        </tr>
        ...
        <tr style="height: 40px;">
            ...
            <td class="table-header col-tanggal" style="width: 100px; text-align: center; vertical-align: middle; border: 2px solid #000000 !important; font-weight: bold; padding: 8px 4px; height: 40px;">TANGGAL</td>
        ...
        <tr>
            ...
            <td class="table-data-center" style="width: 100px; text-align: center; vertical-align: middle; border: 1px solid #000000 !important; padding: 4px;">{{ \Carbon\Carbon::parse($data->tanggal)->format('d/m/Y') }}</td>
            <td class="table-data-left" style="width: 240px; text-align: left; vertical-align: middle; border: 1px solid #000000 !important; padding: 4px; padding-left: 8px;">{{ $data->siswa->nama_lengkap }}</td>
            ...
            <td class="table-data" style="width: 60px; text-align: center; vertical-align: middle; border: 1px solid #000000 !important; padding: 4px;">{{ \Carbon\Carbon::parse($data->jam_terlambat)->format('H:i') }}</td>
            <td class="table-data-center" style="width: 140px; text-align: center; vertical-align: middle; border: 1px solid #000000 !important; padding: 4px;">{{ $data->alasan_terlambat ?? '-' }}</td>
        </tr>
        @endforeach
        ...
        @if($uniqueSanctions->count() > 0)
        <tr>
            <td>&nbsp;</td>
            <td class="footer-text">SANKSI/TINDAKAN :</td>
            <td colspan="3" class="footer-text"></td>
        </tr>
        @foreach($uniqueSanctions as $sanksi)
            @php
                // Split sanksi by newlines to handle multi-line sanctions
                $sanksiLines = explode("\n", trim($sanksi));
            @endphp
            @foreach($sanksiLines as $line)
                @if(trim($line) != '')
                <tr>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td class="footer-text" colspan="3">{{ trim($line) }}</td>
                </tr>
                @endif
            @endforeach
        @endforeach
        ...
        <tr>
            <td>&nbsp;</td>
            <td class="footer-text">SANKSI/TINDAKAN :</td>
            <td colspan="4" class="footer-text">Telat Satu kali = Peringatan</td>
        </tr>
        <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td class="footer-text" colspan="4">Telat Dua kali = Potong rambut</td>
        </tr>
        @endif
        <tr>
            <td colspan="6">&nbsp;</td>
        ...
        <!-- No Data Message -->
        <tr>
            <td colspan="6" class="merge-center" style="text-align: center; padding: 20px 0; border: 1px solid #000000 !important;">TIDAK ADA DATA SISWA TERLAMBAT PADA PERIODE TERSEBUT</td>
        </tr>
        <tr>
            <td colspan="6">&nbsp;</td>
        </tr>
