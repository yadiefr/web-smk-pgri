...
<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Informasi lengkap {{ $user->name }}</p>
        </div>
        <div class="mt-4 md:mt-0 flex space-x-3">
            <a href="{{ route('admin.users.edit', $user) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors">
                <i class="fas fa-edit mr-2"></i>Edit
            </a>
            <a href="{{ route('admin.users.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                <div class="text-center">
                    <img src="{{ $user->photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=3b82f6&color=ffffff&size=120' }}" alt="{{ $user->name }}" 
                    ...
                    <br>
                    <span class="inline-block {{ $user->status == 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} text-sm font-medium px-3 py-1 rounded-full">
                        {{ ucfirst($user->status) }}
                    </span>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        <a href="{{ route('admin.users.edit', $user) }}" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg flex items-center justify-center transition-colors">
                        ...
                        @if($user->id !== auth()->id())
                        <button type="button" class="w-full {{ $user->status == 'aktif' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600' }} text-white px-4 py-2 rounded-lg flex items-center justify-center transition-colors" 
                                onclick="toggleStatus({{ $user->id }})">
                            <i class="fas fa-toggle-{{ $user->status == 'aktif' ? 'off' : 'on' }} mr-2"></i>
                            {{ $user->status == 'aktif' ? 'Nonaktifkan' : 'Aktifkan' }}
                        </button>
                        
                        <button type="button" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg flex items-center justify-center transition-colors" onclick="deleteUser({{ $user->id }})">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 text-base">{{ $user->nip ?? '-' }}</p>
                        </div>
                        <div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <p class="text-gray-900 text-base">{{ $user->address ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-gray-900 text-base">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ ucfirst(str_replace('_', ' ', $user->role)) }}</span>
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                            <p class="text-gray-900 text-base">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->status == 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ ucfirst($user->status) }}
                                </span>
                            </p>
                        </div>
                        <div>
                            ...
                            <p class="text-gray-900 text-base">{{ $user->created_at->format('d F Y, H:i') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Terakhir Diupdate</label>
                            <p class="text-gray-900 text-base">{{ $user->updated_at->format('d F Y, H:i') }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 text-base">{{ $user->last_login_ip ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-4">
                        @if($user->last_login_at)
                            <div class="flex items-center">
                                ...
                                <span class="text-gray-700">Login terakhir: {{ \Carbon\Carbon::parse($user->last_login_at)->diffForHumans() }}</span>
                            </div>
                        @endif
                        
                        <div class="flex items-center">
                            <i class="fas fa-calendar-plus text-blue-500 mr-3"></i>
                            <span class="text-gray-700">Bergabung sejak: {{ $user->created_at->diffForHumans() }}</span>
                        </div>
                        
                        <div class="flex items-center">
                            <i class="fas fa-edit text-yellow-500 mr-3"></i>
                            <span class="text-gray-700">Terakhir diupdate: {{ $user->updated_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            </div>
