...
<div class="container-fluid px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('admin.ujian.bank-soal.import') }}" 
            ...
            <a href="{{ route('admin.ujian.bank-soal.create') }}" 
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ isset($soal) ? $soal->total() : 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ isset($totalAktif) ? $totalAktif : 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ isset($totalPilihanGanda) ? $totalPilihanGanda : 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ isset($totalEssay) ? $totalEssay : 0 }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center space-x-2">
                            <span class="text-sm text-gray-500">{{ isset($soal) ? $soal->total() : 0 }} soal tersedia</span>
                        </div>
                    </div>
                </div>
                            ...
                            <select id="mata_pelajaran" name="mata_pelajaran" 
                                ...
                                @if(isset($mataPelajaran))
                                    @foreach($mataPelajaran as $mp)
                                        <option value="{{ $mp->id }}" {{ request('mapel') == $mp->id ? 'selected' : '' }}>
                                            {{ $mp->nama }}
                                        </option>
                                    @endforeach
                                @endif
                            ...
                            @if(isset($soal) && $soal->count() > 0)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ ($soal->currentPage() - 1) * $soal->perPage() + $index + 1 }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $item->mataPelajaran->nama ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                                            {{ $item->jenis_soal == 'pilihan_ganda' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ $item->jenis_soal == 'pilihan_ganda' ? 'Pilihan Ganda' : 'Essay' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 max-w-md truncate" title="{{ strip_tags($item->pertanyaan) }}">
                                            {{ Str::limit(strip_tags($item->pertanyaan), 60) }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-3">
                                            <a href="{{ route('admin.ujian.bank-soal.show', $item->id) }}" 
                                               class="text-blue-600 hover:text-blue-700 transition-colors duration-150" title="Detail">
                                                <i class="fas fa-eye w-4 h-4"></i>
                                            </a>
                                            <a href="{{ route('admin.ujian.bank-soal.edit', $item->id) }}" 
                ...
                <div class="bg-white px-6 py-3 border-t border-gray-200">
                    <div class="flex justify-between items-center">
                        <div class="text-sm text-gray-700">
                            ...
                            <span class="font-medium">{{ $soal->total() }}</span> hasil
                        </div>
                        <div>
...
<script>
    ...
    function confirmDelete(id) {
        ...
        form.action = `{{ route('admin.ujian.bank-soal.destroy', '') }}/${id}`;
