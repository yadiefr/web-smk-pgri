...
@section('title', 'Jadwal Ujian - Mode Tabel')
...
<div class="container-fluid px-4 py-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <input type="date" id="start_date" 
                       ...
                       min="{{ date('Y-m-d') }}" value="{{ date('Y-m-d', strtotime('+1 week')) }}">
            </div>
    ...
    <div id="tableMode" class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <form id="bulkScheduleForm" action="{{ route('admin.ujian.jadwal.store') }}" method="POST">
            @csrf
...
<script>
let rowCounter = 0;
const mataPelajaranData = @json($mataPelajaran ?? []);
...
// Toggle view
function toggleView() {
    window.location.href = '{{ route("admin.ujian.jadwal.create") }}';
}
