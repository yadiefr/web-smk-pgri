                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            @if(count($jam_slots) > 0)
                                ...
                                <tr class="hover:bg-gray-50 {{ $isBreakTime ? 'bg-amber-50' : '' }}">
                                    <td class="px-2 py-2 text-sm font-bold text-gray-900 border-r border-gray-200 bg-gray-50 sticky left-0 {{ $isBreakTime ? 'bg-amber-100' : '' }}">
                                        <div class="flex items-center justify-center" data-time-slot="{{ $jam_key }}" data-jam-ke="{{ $all_time_slots[$jam_key]['jam_ke'] ?? '' }}" data-is-break="{{ isset($all_time_slots[$jam_key]['is_break']) && $all_time_slots[$jam_key]['is_break'] ? 'true' : 'false' }}">
                                            @if($isBreakTime)
                                                <span class="text-amber-600 font-medium"><i class="fas fa-coffee mr-1"></i>{{ $breakLabel }} ({{ $jam_key }})</span>
                                            @else
                                ...
                                <tr>
                                    <td colspan="{{ count($hari_list_table) + 1 }}" class="px-4 py-12 text-center text-gray-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <button onclick="openCreateModal('{{ request("kelas_id") ?? "" }}' || null, '{{ $selectedHari ?? "" }}' || null)" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm transition-colors">
...
function addJadwalForSlot(hari, jamKey) {
    ...
    // Get current filter values
    const currentKelasId = '{{ request("kelas_id") }}' || null;
