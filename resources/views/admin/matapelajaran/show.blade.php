...
@section('title', 'Detail Mata Pelajaran - SMK PGRI CIKAMPEK')
                ...
                <div class="relative z-10">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                        ...
                        <div class="mt-4 md:mt-0 flex space-x-3">                        
                            <a href="{{ route('admin.matapelajaran.edit', $mapel->kode) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-sm hover:shadow-md transition-all flex items-center">
                                <i class="fas fa-edit mr-2"></i>
                                Edit
                            </a>
                            <form action="{{ route('admin.matapelajaran.destroy', $mapel->kode) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center space-x-3">                        
                            ...
                            <h2 class="text-xl font-bold text-gray-800">{{ $mapel->nama }}</h2>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">                    
                        <div>
                            ...
                            <p class="text-gray-800 font-medium">{{ $mapel->kode }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 mb-1">Guru Pengajar</h3>
                            @if($assignedGuru && $assignedGuru->count() > 0)
                                <div class="space-y-1">
                                    @foreach($assignedGuru as $guru)
                            ...
                            <div class="mb-3">
                                ...
                                @endphp
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($expandedKelasNames as $kelas)
                                            <span class="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md text-xs">{{ $kelas }}</span>
                                        @endforeach
                                    </div>
                                @php
                            ...
                            @if($assignedKelas && $assignedKelas->count() > 0)
                                <div>
                        ...
                        <div>
                            ...
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                   {{ $mapel->jenis == 'Wajib' ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-800' }}">
                                {{ $mapel->jenis }}
                            </span>
                        </div>                        <div>
                            <h3 class="text-sm font-medium text-gray-500 mb-1">Jenis Mata Pelajaran</h3>
                            <p class="text-gray-800 font-medium">
                                @if($mapel->jenis == 'Kejuruan')
                        ...
                        <div>
                            ...
                            <p class="text-gray-800 font-medium">{{ $mapel->tahun_ajaran }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 mb-1">Jam Pelajaran per Minggu</h3>
                            <p class="text-gray-800 font-medium">{{ $mapel->jam_pelajaran }} Jam</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 mb-1">KKM</h3>
                            <p class="text-gray-800 font-medium">{{ $mapel->kkm }}</p>
                        </div>
                    </div>                
                    <div class="mt-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center text-sm">
                                ...
                                <div class="ml-3">
                                    ...
                                    <p class="font-medium text-gray-800">{{ $mapel->updated_at->format('d M Y') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center text-sm">
